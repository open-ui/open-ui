    ...
    $ui.color = {
        ...
        random: function(count, hex1, hex2) {
			...
			if(hex1 && hex2){
			 var hsv1=this.hex2hsv(hex1),
				 ...
				 h=$ui.getRand(Math.min.apply(Math, [hsv1[0], hsv2[0]]), Math.max.apply(Math, [hsv1[0], hsv2[0]]));
				 s=$ui.getRand(Math.min.apply(Math, [hsv1[1], hsv2[1]]), Math.max.apply(Math, [hsv1[1], hsv2[1]]));
				 v=$ui.getRand(Math.min.apply(Math, [hsv1[2], hsv2[2]]), Math.max.apply(Math, [hsv1[2], hsv2[2]]));
			}else{
				h=$ui.getRand(0,360);
				s=100;
				v=$ui.getRand(75,100);
        ...
        */		
		percentage:function(perc, hex1, hex2){
			...
			return $ui.color.hsv2hex([Math.round(hsv1[0]+perc*(hsv2[0]-hsv1[0])),Math.round(hsv1[1]+perc*(hsv2[1]-hsv1[1])),Math.round(hsv1[2]+perc*(hsv2[2]-hsv1[2]))]);
		},		
		/**
